...
(`SNL1`), so an identifier one version suffix or one last segment off
...
for _row in sn_tables.ROWS:
    rule(_row["id"], kind="template", prio="MUST",
         ...
         spec="%s, %s" % (sn_tables.TEMPLATE_CITATION, qualifier_said(_row)),
         fix=_row["fix"],
...
install_near_miss_lint("SNL1", sn_tables)
