...
def _quoted_lines(lines, elision):
    ...
    the line they came from -- a new one starts at any run drawn left
    of the text column, which is where a label or a prompt sits and
    where a continuation does not -- and the label is kept, because
    `fix` and `fix:` are
...
def test_the_banner_carries_no_number_that_can_go_stale(name):
    ...
    if name == "door.svg":
        ...
        return
    # The verdict shot is parametrised in here too and nothing was said
    # about it: the numbers inside the quoted lines are checked against
    # a live run by the test above, but the chrome around them -- the
    # window caption -- is drawn by hand and regenerated by nobody.
    caption = [row for row in re.findall(r">([^<>]+)<", svg)
               if "real output" in row]
    assert caption, "the terminal shot lost its caption"
    assert not re.search(r"\d", " ".join(caption)), (
        "the caption says %r, and a number there is one nothing "
        "regenerates" % caption)
