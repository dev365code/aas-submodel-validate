...
| `scopeNotExamined` | array of object | One object per row of a template table whose scope this run never opened, as `where` (the path the scope sits at), `rule` (the container row's id), `label` (its name in the template), `rulesNotAskedHere` (the ids beneath it that went unasked) and `because`. `because` is `absent` where nothing in that scope matched the row and nothing there was unplaceable either — the file does not carry this section, which for an optional row is not a defect and for a required one has already drawn its own finding — or `unclaimed-element-present` where the row was not entered *and* some element in that same scope matched no row. **The second is two facts side by side and not a cause.** A different identifier may be a legitimate extension (`docs/divergences.md` #19) and nothing distinguishes that from a typo by looking (#22, #23), so this names no culprit and `rulesNotAsked`/`unmatchedElements` above keep their narrower trigger. This is the same reach reported without the blame: before it existed, a container wearing an identifier the template does not name passed with every published number identical to a clean run. **Not a claim about the file and it moves no verdict**, and the screen prints only the `unclaimed-element-present` ones because the other kind is on nearly every clean run. Additive under `schemaVersion` 1. |
