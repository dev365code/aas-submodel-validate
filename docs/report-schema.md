...
| `unmatchedElements` | array of object | The same loss as `rulesNotAsked`, with the element that explains it named: one object per element whose `semanticId` matched no row while rules beneath an unentered row went unasked, as `subject` (where it sits), `seen` (the identifier it carries), `rulesNotAskedHere` (the ids it kept from being put -- named apart from the run total above, because the per-element arrays are neither a partition nor a cover of it and adding them up is not a number of anything) and `resembles` (the row identifier the near-miss lint said it was close to -- always present, because that lint firing is what records the element at all). `rulesNotAsked` above answers *how many*; this answers *which element did it*, which is the question a reader has once the count is not zero. Recorded only where the near-miss lint already reported that this identifier is close to a row's -- that is the sole trigger, and a typo inside a path segment that is not the last draws no lint and so appears here not at all (`docs/divergences.md` #22, #23) -- because the template states a minimum and not a whitelist (#19) and an element of the supplier's own, resembling nothing, explains no absence. **Not a claim about the file and it moves no verdict**: a pipeline that wants to fail on it reads it here, and no flag was added to make it a verdict. Additive under `schemaVersion` 1. |
