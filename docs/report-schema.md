...
| `complete` | boolean | Whether everything this run was handed was read and built. `false` means an archive that would not open, a relationship chain that went nowhere, a part that would not parse or could not be built, or a document over the reader's bound — what did not come out was not judged, and a report that only said `ok: false` could not tell you which. |
...
Every text field of a finding — `message`, `subject`, `detail`, `fix`, `spec` — is bounded at 1000 characters. A report repeats what a file said, and a file can say a great deal: a 200 KB `File` value produced a 200,670-character report before the bound existed. Where a field was cut it says so and by how much (`... (199000 more characters, not shown)`), so a short value and a shortened one are never the same thing on the page. The bound sits far above anything this tool writes — the longest remedy it ships is 403 characters — so it can only ever cut what a file supplied.
