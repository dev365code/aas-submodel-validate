...
four times what a bare document may. The bound is on bytes read, not on
the memory a parse then costs, and it does not cover a ZIP's own
directory. What it refuses to read, it does not judge, and the report
says so. Validation performs no network access.
